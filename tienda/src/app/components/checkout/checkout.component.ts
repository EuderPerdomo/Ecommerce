...
/****Mi mercado PAGO */
public data= {
  "items": [ //PRODUCTOS QUE SE COMPRARAN CON ESTOS DATOS DE PRECIO SE HARA EL CALCULO DEL TOTAL A PAGAR
      {
          "title" : "Producto 1",
    ...
    this._guestService.createPreference(this.data).subscribe(
        ...
        response=>{
          console.log(response)
            ...
            //sandbox_init_point PARA PAGOS FICTICIOS

            //CON ESTA LINEA REDIRECCIONAMOS AL CLIENTE AL FORMULARIO DE PAGO CON MERCADO PAGO
            window.location.href =response.sandbox_init_point; 
        }
    );
